...
int main(int argc, char ** argv)
{
	...
	if (rank == 0) {
		...
		for (i=1; i<size; i++)
			MPI_Send(&global_array[n_elem_per_process*i], n_elem_per_process, MPI_INT, i, 0, MPI_COMM_WORLD);
		...
		// Recibimos los arrays ordenados del resto de procesos y los almacenamos en la matriz
		for (i=1; i<size; i++){
			MPI_Recv(temp, n_elem_per_process, MPI_INT, i, 0, MPI_COMM_WORLD, &status);
		...
		// Recibe los datos
		MPI_Recv(array_to_sort, n_elem_per_process, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		
		// Ordena el array recibido
		sort_array(array_to_sort, n_elem_per_process);
		
		// Envía el array ordenado al proceso "maestro"
		MPI_Send(array_to_sort, n_elem_per_process, MPI_INT, 0, 0, MPI_COMM_WORLD);
	}
