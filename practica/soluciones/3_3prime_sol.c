...
int main (int argc,  char *argv[]){
  ...
  if (world_rank == ROOT) {
    ...
    pc = 4;                                                                           /* COMPLETAR (1 línea). Línea no crítica */
    for (i = start; i <= LIMIT; i = i + stride) {
      if (is_prime(i)) {
        pc++;									                                                        /* COMPLETAR (1 línea) */
        found_prime = i;							                                                /* COMPLETAR (1 línea) */
      }
    }
    MPI_Reduce(&pc, &pcsum, 1, MPI_INT, MPI_SUM, ROOT, MPI_COMM_WORLD);               /* COMPLETAR (1 línea). Usa MPI */
    MPI_Reduce(&found_prime, &max_prime, 1, MPI_INT, MPI_MAX, ROOT, MPI_COMM_WORLD);  /* COMPLETAR (1 línea). Usa MPI */
    end_time = MPI_Wtime();
